...
if(!$result){
    ...
    JOIN \"Administrator\" USING (\"ID_individual\") WHERE \"ID_administrator\"=$editid";
    $individual = resultNumber($individual, $db_connect);
    
    $account="
    SELECT \"ID_account\" FROM \"Account\"
    JOIN \"Administrator\" USING (\"ID_account\") WHERE \"ID_administrator\"=$editid";
    ...
    $sql="
    UPDATE \"Address\" SET country city street house flat
    WHERE (\"ID_address\"=$address)";

    $stmt = $db_connect->prepare($sql);
    if(!empty($country)){
        $stmt->bindValue("country", "\"Country\"='$country'");
    }if(!empty($city)){
        $stmt->bindValue("city", ",\"City\"='".$city."'");
    }if(!empty($street)){
        $stmt->bindValue("street", ",\"Street\"='".$street."'");
    }if(!empty($house)){
        $stmt->bindValue("house", ",\"House\"='".$house."'");
    }if(!empty($flat)){
        $stmt->bindValue("flat", ",\"Flat\"='".$flat."'");
    }
    $stmt->execute();
