    ...
    if (!$result) {
        ...
        if ($address !== false) {
            ...
            if ($individual !== false) {
                ...
                if ($account !== false) {
                    // Admin Insertion
                    $admin_query = "INSERT INTO public.\"Administrator\" VALUES (DEFAULT, $account, $individual) RETURNING \"ID_administrator\"";
                    $admin = insertWithRetry($admin_query, $db_connect, 'Administrator', 'ID_administrator');

                    if ($admin !== false) {
                        echo 'Добавление прошло успешно';
                    } else {
                        echo 'A critical database error occurred, failed to register the admin after several attempts.';
                    }
                } else {
