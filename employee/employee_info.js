$("#info_employee").on("click", function( event ) {
    event.preventDefault();
    employee_info();
});

var deleteData_em = function(id){
    $.ajax( {
      type: 'POST',
      url: 'vendor/employee_del.php',
      data:{
        deleteid:id
      },
      dataType: 'text'
...
var editData_em = function(id){
    form_employee_ch(id);
};

$("#main_content").on("submit", "#employee_edit", function( event ) {
    event.preventDefault();
    $.ajax( {
        type: 'POST',
        dataType: 'text',
        url: 'vendor/employee_edit.php',
        data:{
            editid:$("#editid").val(),
    ...
    .done(function (data, textStatus, jqXHR) { 
        if (data=='0'){
            ...
            employee_info();
            alert("Запись о сотруднике с номером " + data + " успешно изменена");
        }
    })
    .fail(function (jqXHR,textStatus) {
        alert("При обращении к серверу возникли проблемы: " + textStatus);
...
function employee_info () {
    $.ajax( {
        type: 'POST',
        dataType: 'json',
        url: 'vendor/employee_info.php'
    })
    .done(function (data, textStatus, jqXHR) { 
        $('#main_content').empty();
        $('#main_content').append(`
            <h1 class="title">Сведения о сотрудниках</h1>
            
            <div class="table-data">
                <div class="order">
                    <div id="all_employee_accounts">` + parse_json_ch_em(data) + `</div>
                </div>
            </div>
        `);
    })
...
function parse_json_ch_em (data) {
    var table = '<table>';
    var thead = '<thead><tr>';
    var i = 0;
    ...
    $(data).each(function(index, element) { 
      if (i > 0){
        ...
        <td class="edit" style="margin-right: 15px;">
            <div class="edit-d">
                <a href='javascript:void(0)' onclick='editData_em(` + data[index][0] + `)'><i class='bx bxs-edit'></i></a>
            </div>
            <div class="delete-d">
                <a href='javascript:void(0)' onclick='deleteData_em(` + data[index][0] + `)'><i class='bx bxs-trash-alt'></i></a>
            </div>
        </td>
        `;
...
function form_employee_ch(id){
    ...
    $('#main_content').append(`
        <h1 class="title">Форма изменения данных сотрудника</h1>
        
        <div class="table-data">
			<div class="order">
                <div class="conteiner-form">
                    <div class="findent">
                    <form id="employee_edit" class="form_em" action="#">
                        <div class="form first">
                            ...
                            <div class="details ID">
                                ...
                                <div class="fields">
                                    <div class="input-field">
                                        <label>Логин</label>
                                        <input id="login" type="text" placeholder="employee123" >
                                    </div>
                                    <div class="input-field">
                                        <label>Пароль</label>
