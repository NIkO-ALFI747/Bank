...
if(!$result){
    ...
    JOIN \"Employee\" USING (\"ID_individual\") WHERE \"ID_employee\"=$editid";
    $individual = resultNumber($individual, $db_connect);

    $account="
    SELECT \"ID_account\" FROM \"Account\"
    JOIN \"Employee\" USING (\"ID_account\") WHERE \"ID_employee\"=$editid";
    ...
    $query="
    UPDATE \"Employee\" SET \"Employee_type\"='$employee_type' WHERE (\"ID_employee\"=$editid) AND ('$employee_type'<>'')"; 
    $db_connect->query($query);
    if(!empty($ID_office)){
        $query="
        UPDATE \"Employee\" SET \"ID_office\"=$ID_office WHERE (\"ID_employee\"=$editid)"; 
        $db_connect->query($query);
    }
