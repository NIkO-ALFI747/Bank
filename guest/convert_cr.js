...
$("#main_content").on("keyup","#selected_currency_price_2", function() {
    $.ajax( {
        type: 'POST',
        url: '../admin/queries/calc_exchange.php',
        data: {
...
function pop_course(){
    $.ajax( {
        type: 'POST',
        dataType: 'json',
        url: '../admin/queries/m11_pop_course.php'
    })
    .done(function (data, textStatus, jqXHR) { 
        $('#main_content').append(`
...
function form_exchange_card(select){
    return `
        <div class="table-data">
			<div class="order">
                <div class="head">
                    <h5>Калькулятор конвертирования валют для продажи и покупки</h5>
                </div>
                <div class="conteiner-form">
                    <div class="find_card_2">
                    <form id="employee_edit" class="currencies" action="#">
