			...
			<div class="order">
                ...
                <div class="conteiner-form">
                    ...
                    <form id="customer" class="form_cs" action="#">
                        <div class="form first">
                            <input id="ID_credit_card" type="hidden" value="`+id+`">
...
$("#main_content").on("submit", "#customer_card", function( event ) {
    ...
    .done(function (data, textStatus, jqXHR) { 
        $('#message').empty();
        $('#message').append('<div style="color:#ffe41c;">Кредитная карта успешно оформлена. Клиент зарегистрирован.</div>');
    })
    .fail(function (jqXHR,textStatus) {
        alert("При обращении к серверу возникли проблемы: " + textStatus);
    })
...
$("#main_content").on("submit", "#customer", function( event ) {
    ...
    .done(function (data, textStatus, jqXHR) { 
        form_open_card($("#ID_credit_card").val(),data);
    })
    .fail(function (jqXHR,textStatus) {
        alert("При обращении к серверу возникли проблемы: " + textStatus);
    })
